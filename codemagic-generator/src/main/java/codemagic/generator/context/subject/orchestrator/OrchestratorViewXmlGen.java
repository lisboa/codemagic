...
public class OrchestratorViewXmlGen {

	private final OrchestratorContext orchContext;
	
	public OrchestratorViewXmlGen(final IContext ctx) {
	...
	public void generate() throws IOException {
		...
		// input
		final String template = ClassLoadUtil.loadFile(OrchestratorViewXmlGen.class, "templates/orchestrator/_BaseObjectName_View.ui.xml.template");
		
		final Engine engine = new Engine();
		
		final String transformedJava = engine.transform(template, orchContext.getModel());
		
		final String fullName = orchContext.buildArtifactFullFileName( ArtefactyType.VIEW_XML );
